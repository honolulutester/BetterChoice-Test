...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    const hashPage = window.location.hash.replace("#", "") || "shop";
    if (pid && state.activeProductTrace) {
        state.activePage = "traceability";
        renderPage("traceability");
    } else {
        renderPage(hashPage === "traceability" && !state.activeProductTrace ? "shop" : (hashPage || state.activePage));
    }
});
...
function registerUser({ name, email, phone, password, address, area }) {
...
function issueEcoCredits(email, amount) {
    const user = state.users.find(u => u.email === email.trim().toLowerCase());
...
function setupRouting() {
    ...
    window.addEventListener("hashchange", () => {
        const page = window.location.hash.replace("#", "") || "shop";
    ...
    // Set initial route based on hash (if any)
...
function renderLoginView(container) {
    ...
    forgot.innerHTML = `<a href="#" id="forgot-hint">Forgot password?</a> — contact Gulshan desk with your registered email.`;
...
function renderTraceabilityView(container) {
    ...
    const related = state.products
        ...
        .slice(0, 4);
    const reviews = state.productReviews[p.id] || [];
...
function renderDashboardView(container) {
    ...
    const renderTab = (tabName) => {
        ...
        if (tabName === "profile") {
            ...
            `;
            document.getElementById("return-request-form").addEventListener("submit", (e) => {
                e.preventDefault();
            ...
            document.getElementById("advance-order-form").addEventListener("submit", (e) => {
                e.preventDefault();
...
function renderCheckoutForm(container) {
    ...
    document.getElementById("place-order-btn").addEventListener("click", () => {
        ...
        if (paymentMethod === "cod") {
            finalizeTransaction(totals.finalTotal, "Zero-Plastic Cart Checkout", details.slot, 0, totals.storeDeduction, details, "Cash on Delivery");
            return;
        }
